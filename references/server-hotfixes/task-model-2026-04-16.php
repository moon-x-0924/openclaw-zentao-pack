...
{
    /**
     * 向本机 webhook bridge 推送任务事件兜底通知。
     * Post task event to local webhook bridge as a fallback for realtime notification.
     *
     * @param  int    $taskID
     * @param  string $action
     ...
     * @return void
     */
    protected function postTaskWebhookEvent($taskID, $action, $comment = '', $actor = '')
    {
        $taskID  = (int)$taskID;
        $action  = strtolower(trim((string)$action));
        $comment = (string)$comment;
        $actor   = $actor ? trim((string)$actor) : $this->app->user->account;
        ...
        $ch = curl_init('http://127.0.0.1:37812/zentao/webhook');
        if(!$ch) return;
    ...
    /**
     * 判断任务变更集中是否包含指定字段。
     * Check whether the task change list contains a target field.
     *
     * @param  array  $changes
     * @param  string $field
     * @access protected
     * @return bool
     */
    protected function hasTaskFieldChanged($changes, $field)
    {
        if(empty($changes) || !is_array($changes)) return false;

        foreach($changes as $change)
        {
            if(is_object($change) && isset($change->field) && $change->field == $field) return true;
            if(is_array($change) && isset($change['field']) && $change['field'] == $field) return true;
        }
    ...
    /**
     * 按任务动作补发状态类 webhook 事件。
     * Post task status webhook event by task action name.
     *
     * @param  int    $taskID
     * @param  string $action
     * @param  string $comment
     ...
     * @return void
     */
    protected function postTaskStatusWebhookByAction($taskID, $action, $comment = '', $actor = '')
    {
        $action = strtolower(trim((string)$action));
        $map = array(
            'started'   => 'started',
        ...
        $event = zget($map, $action, '');
        if(!$event) return;

        $this->postTaskWebhookEvent($taskID, $event, $comment, $actor);
    }
    ...
    public function activate($task, $comment, $teamData, $drag = array())
    {
        ...
        $this->postTaskStatusWebhookByAction($taskID, 'activated', $comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($taskID, 'assigned', $comment);
    ...
    public function afterStart($oldTask, $changes, $left, $output = array())
    {
        ...
        $this->postTaskStatusWebhookByAction($oldTask->id, $action, $this->post->comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($oldTask->id, 'assigned', $this->post->comment);
    ...
    public function assign($task)
    {
        ...
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($task->id, 'assigned', $this->post->comment);
    ...
    public function cancel($oldTask, $task, $output = array())
    {
        ...
        $this->postTaskStatusWebhookByAction($oldTask->id, 'canceled', $this->post->comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($oldTask->id, 'assigned', $this->post->comment);
        return true;
    }
     ...
     * @return false|void
     */
    public function checkEstStartedAndDeadline($executionID, $estStarted, $deadline, $prefix = '')
    {
    ...
    public function close($oldTask, $task, $output = array())
    {
        ...
        $this->postTaskStatusWebhookByAction($oldTask->id, 'closed', $this->post->comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($oldTask->id, 'assigned', $this->post->comment);
    ...
    public function finish($oldTask, $task)
    {
        ...
        $changes = common::createChanges($oldTask, $task);
        $this->postTaskWebhookEvent($oldTask->id, 'finished', $this->post->comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($oldTask->id, 'assigned', $this->post->comment);
    ...
    public function pause($task, $output = array())
    {
        ...
        if(dao::isError()) return false;
        $changes = common::createChanges($oldTask, $task);
        $this->postTaskStatusWebhookByAction($task->id, 'paused', $this->post->comment);
        if($this->hasTaskFieldChanged($changes, 'assignedTo')) $this->postTaskWebhookEvent($task->id, 'assigned', $this->post->comment);
     ...
     * @return object
     */
    public function processTask($task, $convertParent = true)
    {
        $today = helper::today();
