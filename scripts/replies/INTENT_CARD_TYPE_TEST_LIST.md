...
| 13 | `query-task-detail` | `button_interaction` | `任务详情 1` | `开始任务 / 完成任务 / 阻塞任务` | `wait/pause -> 开始任务；doing -> 完成任务+阻塞任务；done/closed -> 无操作按钮，降级只读卡` |
| 14 | `query-bug-detail` | `button_interaction` | `bug详情 1` | `激活Bug / 解决Bug / 关闭Bug` | `active -> 解决Bug+关闭Bug；resolved -> 激活Bug+关闭Bug；closed -> 激活Bug；其他状态 -> 无操作按钮，降级只读卡` |
...
| 30 | `review-story` | `vote_interaction` | `评审需求 1 pass` | `通过 / 驳回 / 需补充；提交评审` | `story.status=closed 时不再展示可提交投票，降级只读卡` |
| 31 | `create-task` | `text_notice` | `创建任务` | `-` | `-` |
| 32 | `update-task-status` | `multiple_interaction` | `更新任务状态 1 doing` | `选择任务状态；待处理 / 进行中 / 已完成 / 已阻塞；备注策略；使用默认备注 / 不写备注；提交更新` | `wait/pause -> 仅进行中；doing -> 仅已完成+已阻塞；done/closed -> 无合法选项，降级只读卡` |
...
| 39 | `update-bug-status` | `multiple_interaction` | `更新bug状态 1 resolve` | `选择Bug状态；激活 / 已解决 / 已关闭；备注策略；使用默认备注 / 不写备注；提交更新` | `active -> 仅已解决+已关闭；resolved -> 仅激活+已关闭；closed -> 仅激活；其他状态 -> 无合法选项，降级只读卡` |
